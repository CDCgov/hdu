# FHIR Recognizer (fhirec.py)

A command-line utility and Python module for discovering FHIR endpoints and related API metadata.

## Overview

`fhirec.py` searches for FHIR and related endpoints on a given URL/host, including:
...
```bash
python fhirec.py --url https://example.com/fhir
```
...
**Using a full URL:**
```bash
python fhirec.py --url https://example.com/fhir/metadata
```

**Using hostname and prefix separately:**
```bash
python fhirec.py --hostname https://example.com --fhir_prefix /fhir
```

**Include detailed results:**
```bash
python fhirec.py --url https://example.com/fhir --all
```
...
```python
from fhirec import fhir_recognizer
```
